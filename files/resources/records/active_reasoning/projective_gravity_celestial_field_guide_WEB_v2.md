...
**The surface-density coincidence, three ways [V].** For the Milky Way, with the disk's central surface density extrapolated from its exponential falloff ($R_d\approx2.6$ kpc, $47\,M_\odot/{\rm pc}^2$ at the Sun, so $\Sigma_0\approx1100\,M_\odot/{\rm pc}^2$), one finds $a_0/(G\Sigma_0) = 0.78$: **$a_0 \approx G\Sigma_0^{\rm central}$ with coefficient of order one.** This is consistent with the closure derivation $a_0 = 4\pi G\Sigma_{\rm mean}$ exactly when the hemisphere edge sits at $x^\ast = 4.90$ disk scale lengths — $12.7$ kpc for the Milky Way — which puts the 3-sphere radius at $\ell = 2R_{\rm edge}/\pi = 8.1$ kpc. Four independent-looking routes then land at the same number: the MOND radius $r_M = \sqrt{GM/a_0} = 8.3$ kpc; the Entry-6 radius $\sqrt{GM/cH_0} = 8.0$ kpc; the closure radius $8.1$ kpc; and the Sun's own orbit, $8.2$ kpc — so the Sun sits at colatitude $\chi = 1.01$ rad. The first two are Milgrom's $a_0\approx cH_0/2\pi$ in two costumes; the third is partly independent; the fourth is the known fact that the Milky Way crosses $a_0$ at the solar circle. The *coefficient* of the coincidence is not $2\pi$ but depends on which surface density is meant — $1\times$ central, $\sim2\pi\times$ Freeman-typical, $4\pi\times$ mean — and the closure derivation is the one that says which: the mean, out to the equator.

**The problem that does not go away [O].** With $\ell\approx8$ kpc the equator is at $12.6$ kpc, and beyond it the closure curve *rises* toward the twin. Gaia's rotation curve is flat-to-slightly-declining from 5 to 25 kpc. No rise. The scale problem of the rotation-curve family is the same problem, and the surface-density coincidences do not remove it.
...
**Hubble's law from our own galaxy (Entry 6) — the wall, and where the $10^6$ lives [V/D].** The proposal: the antipode inherits the galaxy's gravitational structure, light paths pass it, and something there does the redshifting — with a "generalization of how the metric applies" letting a path ending on the visible galaxy inherit the antipode's static properties. The wall is quantitative and it is B's wall: the galaxy's potential depth at $r = a$, and its twin's identically, is $GM/c^2a = aH_0/c\approx10^{-6}$, against cosmological $z$ of $10^{-2}$ to $10$. Short by $10^4$–$10^7$, the same factor by which the twin's lensing is too weak — because every gravitational effect the twin can produce is set by the one number $GM/c^2r\sim10^{-6}$, and order-unity effects need a horizon. Inheritance does not change this: the antipode's metric properties *are* the galaxy's, and the static theorem keeps redshift endpoint-only regardless. **But the $10^6$ is not absent from the model.** The Mach lock at galaxy scale gives $M_{\rm tot} = \pi c^2\ell/2G\approx3\times10^{17}M_\odot$, four million galaxies, with $GM_{\rm tot}/c^2\ell = \pi/2\sim1$ — order-unity gravity at the 3-sphere scale, by construction. It cannot be uniform (we would feel it), and if it is concentrated in the dark hemisphere the universe is not static and the galaxy falls toward it — which is precisely the non-staticity the redshift theorem demands. A door, into "we orbit a horizon-scale dark mass in a closed space," not into the picture as drafted.
...
**A candidate law, with the right form and a wrong coefficient [V/O].** The early relation $GM_{\rm gal} = c\,a^2H_0$ — "the galaxy's acceleration at the 3-sphere scale equals $cH_0$" — gives $a\propto\sqrt M$, exactly the form II.3 needs, and identifies $a$ with the MOND transition radius, $a = r_M/\sqrt{2\pi}$. But used directly as $\ell$ in the plateau it predicts $a_0 = (16/\pi^2)\,cH_0$, $8.8\times$ too large. The Tully–Fisher-consistent radius is $\ell = (4/\pi)r_M = 3.2\,a$, and $3.2\approx\pi$: with $\ell = \pi a$ the prediction becomes $a_0 = 16cH_0/\pi^4 = 0.164\,cH_0$, against the observed $0.183$ and Milgrom's $1/2\pi = 0.159$ — within 10%. Whether "$\ell = \pi a$" means anything or is numerology is exactly the kind of question this part exists to hold.

**The surface-density invariant (Entry 7).** The proposal that mass, for galaxies, is *apparent* — post scale-inflation — and that the invariant is $M/a^2$, a surface density. With $a$ defined by $GM = ca^2H_0$, $M/a^2 = cH_0/G \approx 4700\,M_\odot/{\rm pc}^2$ is universal *by construction*, so the proposal's content is entirely the identification of $a$ with the 3-sphere scale; the number it produced for the Milky Way, $(M/a^2)/\Sigma_0\approx4.3$ against $cH_0/a_0 = 5.5$, is the $a_0\approx G\Sigma_0$ coincidence of I.6 in another form. The idea that the "$2\pi$" is not fundamental but depends on which surface density is meant is correct (I.6).
...
| $a_0\approx G\Sigma_0$ | Milky Way: $a_0/(G\Sigma_0^{\rm central}) = 0.78$; consistent with $4\pi G\Sigma_{\rm mean}$ at 4.9 scale lengths, $\ell = 8.1$ kpc | [V] |
| four routes to 8 kpc | $r_M$, $\sqrt{GM/cH_0}$, closure $\ell$, the solar radius; $\chi_\odot = 1.01$ rad | [V] |
| Entry-6 law | $GM = ca^2H_0$: right form ($\ell\propto\sqrt M$), $a = r_M/\sqrt{2\pi}$; $\ell = \pi a$ gives $a_0 = 16cH_0/\pi^4$ | [V/O] |
| Hubble-from-galaxy | $GM/c^2a = aH_0/c\approx10^{-6}$ vs $z\sim1$; the $10^6$ lives in $M_{\rm tot}$ | [V] wall |
