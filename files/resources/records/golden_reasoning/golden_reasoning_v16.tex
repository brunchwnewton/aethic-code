% golden_reasoning_v90_presentable.tex -- circulation copy generated from
% the master on 2026-09-01: whole-line source comments (correction
% records, pass records, author-check notes) stripped; body identical.
...
@book{Hume1748, author={Hume, David}, title={An Enquiry Concerning Human Understanding}, year={1748}, note={AUTHOR TO VERIFY edition}}
@misc{BenGradeSchool, author={Benander, Ajax K.}, title={Ajax's Math Ideas (grade-school idea journal)}, howpublished={\url{https://aethus.org/aethic-code/history/ajax-grade-school-ideas.html}}, note={Dated public record; Golden Point entry mid-January 2021}}
@misc{BenGolden22, author={Benander, Ajax K.}, title={Golden Science}, year={2022}, howpublished={\url{https://aethus.org/aethic-code/files/GoldenScience_Nov2022.pdf}}, note={Dated August 2022; posted November 2022}}
...
\bibitem{BenGolden22} A.~K.~Benander, \emph{Golden Science}, August
2022 (posted November 2022),
...
argued separately. The model is one of three movements. The second is a conditioning
...
\item[Miracleism] The meaning tradition of Part~IV, named for its
source: the Miracles fund the meaning; the Golden Point actualizes
it, preventing vanity. Remark~\ref{gs:rem:miracleism}
...
following, which \S\ref{gr1:sec} develops properly. \emph{The hazard.} $h^{\mathrm{tot}}_t=B_t\varsigma_b(x_t)+\Gamma
...
$\tfrac14$. \emph{Departure.} $T$ is the first time the cumulative hazard exhausts an
...
The document is long and its parts are separable. Four entry points. \emph{For the mathematics}, start at \S\ref{gr1:sec} and read to
...
companion program and can be assessed on its own terms. \emph{For the argument about capacity and risk}, read
\S\ref{gr1:sub:cond} for the two-condition theorem and then
\S\ref{hr:sec}, which applies it to extinctionism and to
techno-optimism in four pages. \emph{For the philosophical grounding}, read \S\ref{ac:sec}. It is the
...
(\S\ref{gc:sub:humanhistory}) is the one to read if reading only one. \emph{For the argument that the Golden Point is the right target}, read
...
forward reading of the conditioning argument and states what would, and by theorem what would not, distinguish it from its rivals. It is
...
Two conventions run throughout. Every substantive claim carries a grade, \textbf{[P]} proved, \textbf{[C]} cited, \textbf{[A]} conditional on a
...
\paragraph{Why the second horn is structural, and where it could fail.}\label{ac:rem:structural}
...
\paragraph{What the curse removes, and what it does not.}\label{ac:rem:authority}
...
\paragraph{What $\Phi2$ denies, and what $\Phi6$ claims.}\label{ac:rem:phi2split}
...
question, comparably constrained, delivering a different answer. The companion argues that a reasoner cannot detach from its biosphere
...
argued above, and a rival question would have to match it. Two limits, since the argument is easy to over-draw. It establishes a
...
\paragraph{What agency is, in this framework, and why it is not a
...
nothing that was ever claimed. \emph{Nothing here says the Golden Point is good.}
...
purchased at its stated price is developed in full as the third route, inferred meaning, with its two-tier claim, its literature, and its
...
requires, live stakes, real fragility, something genuinely at risk, is satisfied at \emph{every} time under the affirmative, and what
...
continuation fails to advance, so the Golden Point is unreachable, modally, not merely improbably, and viability is zero. Nothing has yet
...
critical cases exactly backwards. \emph{What replaces it.} The existential dichotomy for a biosphere is
...
\paragraph{The outlook, in one sentence.}\label{ac:rem:race}
...
\paragraph{The register of this section, stated for the reader who
...
load-bearing step, the two-condition theorem, the Filter, the ceiling, and the relation runs one way: \emph{the mathematics limits the
...
is \emph{canonical}, the complement, so nothing is analyst-supplied, and the exceedance form is its own multiplicity guard, bounding the
...
conclusions as conditionals on it. \emph{The companion's own limits transfer with them.} In particular the
...
and \S\ref{gc:sub:naturalhistory} state their arguments in the 2022 Laws, Second Law, Third Law, Golden Linkage, the $L$-threshold, while
...
analysis of \S\ref{gc:subsub:cascade} consumes its closure formalisation, the companion now carries canonical anchors for both, and the
...
relative to it. The difficulty is that observer-existence is \emph{robust} in a way the
...
quantifier restores. Two consequences for how the model of \S\ref{gr1:sec} should be read.
...
hazard is a hazard \emph{of}. The internal and external hazards of \eqref{tc:eq:hazard} \emph{imply}
...
hazards suffice for departure and are not necessary to it. \emph{The two-generation form is the companion's, and was not imported.}
...
reason, by the same result. Two clarifications, since the proposition is easy to over-read.
...
re-arises, that is what the two previous results jointly establish, and a channel whose reduction depends on an agent that no longer
...
makes the framework non-anthropocentric, and the non-anthropocentrism is essential rather than incidental. Begin from what the target is. \emph{The Golden Point is a property of a
...
without the threshold, which is Theorem~\ref{gr1:thm:trapped}(ii) in
...
it'' is the Score of \S\ref{gs:sec} as a germ. An August 2021 entry
...
Remark~\ref{sch:rem:notexhausted}, posed as a subscript four years
before its resolution. The record shows the conception arising from
...
author's \emph{Golden Science} of August 2022 \cite{BenGolden22},
...
two-channel decomposition of \S\ref{gr1:sec}, with the external
...
Remark~\ref{gs:rem:arrivaltiming}; Optimation and the task-object
...
retrocausal accordance (\S\ref{ra:sec}) later paid. The same
...
allow the biosphere to persist past the eventual death of its star. \emph{Step two: perturb the record and watch them fail.} Consider
...
\textbf{[P]}. It inherits the substitution property's own license, rung-screening, flagged as such at the source \cite{Ben26Optimal}, and it establishes \emph{local} optimality among interrogated
...
$1-p$ to occupy $\neg F$'' is a \emph{self-sampling} premise, location weighted within a reference class spanning the $F$- and
...
Accordance bound \eqref{gc:eq:accordance} with $H$ the Golden Point, bounds it quantitatively: $b/a < p/(1-p)$, so the flukier the event, the more
...
The same reductio, run on the natural-historical record. Its premise, that observerhood survives the perturbations considered, is
...
come to gather, conditioned on their consisting with the Golden Point, but outright Aethic disjointness from the Golden Point is
forbidden.
\end{description}
\end{definition}
...
of them and raises its price. By Remark~\ref{sch:rem:asymptote}, the climbing of a schedule is
...
Golden-Point probability. This work keeps those apart, Remark~\ref{gr1:rem:whatTis} insists $\PP(\Lambda_\infty<\infty)$ is the
...
cannot decline. \emph{Refusing Void commits Weak to privileging the Golden Point.} If
...
proper children, so the anchor does real work. That second commitment needs care, because stated carelessly it collides
...
\emph{relative}, a share of a total the hypothesis does not bound, so it inherits the caution of Remark~\ref{sch:rem:absolute} without
...
parallel is worth flagging as an open lead. \emph{Compatibility with the vantage restriction.}
...
individuates, criterion there, construction here, one doctrine, and, under the Route World's Strong reading only, the section closes
...
\paragraph{Root-relativity is by design, not by accident.}\label{to:rem:rootrelative}
...
\paragraph{Why this is not a redescription of ordinary objects.}\label{to:rem:notredescription}
...
persistence in the past and the prospects of a biosphere's near future, and the coincidence is worth recording as a convergence rather than
...
\paragraph{Robustness recovered without substance.}\label{to:rem:robustness}
...
\paragraph{The criterion is a gauge choice, and the paper already
...
\paragraph{The reply, as a reductio of the objection's own criterion.}\label{to:rem:reductio}
...
is reachable by ascending to a sufficiently early ancestor Aethus, plausibly in infancy or before, wherever a substantial portion of the
...
\paragraph{Everything as a fluctuation on the Golden Aethus;
...
\paragraph{A payoff: the fluctuation-observer pathology displaced.}\label{to:rem:boltzbrains}
...
well-formed rather than merely persuasive. \emph{The perturbation arguments.} \S\ref{gc:subsub:pastalign-argument}
...
occur, which is a task-object identification and not an object one. \emph{Imperfection Magnitude Drops.} A drop is a fall in contribution to
...
\paragraph{Order of development.}\label{to:rem:order}
...
as new. \emph{The classical layer.} The state process is Kolmogorov's 1934
...
\emph{The conceptual ancestor.} The thesis of \S\ref{sh:sub:interp}, that selection on survival makes the observed ensemble diverge
...
$\PP(T>t)=\E[e^{-\Lambda_t}]$ and not $e^{-\E[\Lambda_t]}$. The same gap is what Remark~\ref{sch:rem:gap2} records for completion
...
takes the Golden Point, the asymptotic survivorship structure, as its first-principles root: a term is well-defined, in the sense
...
\paragraph{No boundary in $h$.}\label{sh:rem:noboundary}
...
\paragraph{Credit is spent only through negative rate.}\label{sh:rem:monot}
...
\paragraph{Why this matters.}\label{tc:rem:asymmetry}
...
derivative of the hazard, rather than on the hazard or its logarithm, is the model's single most consequential choice, and this subsection
...
\paragraph{Where the ladder stops.}\label{gr1:rem:ladderstops}
The rungs are not equally hospitable, and the model sits on the last one
that is analytically open. Noise at level one, $\log h$ Brownian, makes the credit Brownian and the persistence exponent $\tfrac12$. Noise
...
\paragraph{The two channels, in operator terms, and why they differ.}\label{gr1:rem:channelsoperator}
...
\paragraph{Status of the Ansatz.}\label{sh:rem:ansatzstatus}
...
survival}, each zero crossing killing with probability $p$, carries exponents that vary continuously with $p$
...
\paragraph{How close, in one sentence.}\label{tc:rem:howclose}
...
asymptotics depend on the data only through a single equivalent timescale, is Proposition~\ref{sh:prop:degrees} below.
...
muffling sits \emph{above} rather than below its background hazard, has logarithmic density exactly $\tfrac12$, for every parameter value
...
\paragraph{What the exact half is for, and what it is not.}\label{gr1:rem:halfuse}
...
$\PP(Z_0<-\varepsilon)>0$; here the constant is exact. \emph{Disclaimer, and it is the important one.} This is a statement
...
vanishes at infinity is mixing \cite{Mar49}, hence ergodic. \emph{Step 3 (Birkhoff).} Fix $\varepsilon>0$. Since
...
$\Leb\{s\ge0:Z_s\le-\varepsilon\}=\infty$ a.s. \emph{Step 4 (change of variables).} Let
...
\paragraph{The asymmetry of exponential response.}\label{sh:rem:asym}
The driver is symmetric; the response is not. A favorable excursion
suppresses the integrand $e^{-D}$ multiplicatively toward zero, \emph{bounded} benefit for $\Lambda$, while an unfavorable excursion of
...
aggravated by this asymmetry. Identifying exactly what a horizon must break, the symmetry of the driver, not the response, is the burden of
...
acceleration process} and has an extensive literature of its own, the review \cite{BMS13}.
\end{theorem}

\paragraph{The $m^{1/2}$ law is exact given the $m=1$ asymptotic.}
...
the process instead. The idea, essentially, is that the capacity for us to destroy ourselves
...
\paragraph{Why this is the right home for the notion.}\label{gr1:rem:anatomyhome}
...
exists, $\inf_tB_t=0$ a.s.\ (Remark~\ref{gr1:rem:trappedscope}), and no exponential bound is claimed there; part (i) is what holds
...
alone floors the hazard at $\Gamma e^{-\rho x_t}\ge\Gamma$ there, a deterministic floor on an infinite set, or
...
property of the driver. For $\rho=0$ that is exact: the trough alone floors the hazard at
...
\paragraph{What the trapped regime looks like.}\label{gr1:rem:trapped}
...
\paragraph{What the theorem computes.}\label{gr1:rem:twocondreading}
...
\paragraph{Transience is a feedback loop.}\label{gr1:rem:feedback}
...
outcome is governed by Proposition~\ref{sh:prop:recurrent}, $\bar m>0$ gives $D_t/t\to\bar m$ and $\Lambda_\infty<\infty$, while
...
therefore requires $\rho_t$ to be eventually positive almost surely, transient upward, or absorbed above some $\rho_c>0$, and the
...
\paragraph{This is the event horizon the fixed model could not
...
almost no time at all when strong. \emph{An ordering constraint falls out.} Since the cost of anatomical
...
\paragraph{Where the earlier results now sit.}\label{gr1:rem:wherenow}
...
\paragraph{The expansion fails exactly at the anatomy threshold.}\label{gr1:rem:perturbfail}
...
not an alternative to \eqref{tc:eq:x} but a discounting of it. The second equation is the same commitment one derivative up, and it is
...
gates closed. But the hazard depends on the credit \emph{exponentially}, both channels of \eqref{tc:eq:hazard} do, by construction, so the
...
half-climbed is nearer in hazard to one not begun than to one completed. Set this beside Remark~\ref{hr:rem:monotone}, which has stakes rising
...
speculation laid beside it. Principle~\ref{gc:prin:continuity} holds that hazard-lowering across
...
The author's central move is then an identification. What the
...
\paragraph{The last leg falls on the anatomy (design at \textbf{[S]}, debt at \textbf{[O]}).}\label{sch:rem:anatomyleg}
...
remark's continuity argument removes the workaround without removing
the problem, since the term must be live, and that forces the
resolution rather than merely permitting it: the $x$-directed
component of the schedule must already sit in its exponential-tail
regime by the modern day, with $x$ robust, so the term's remaining
incompleteness before the Golden Point has to act somewhere other
than the credit.

The author's proposal is that it acts on the technoanatomy
directly. On this design the last leg of the schedule pushes $\rho$
around as a trajectory until it settles at a random value drawn from
a settlement distribution, and the interval between the present and
the Golden Point is exactly the crapshoot of pulling this off,
embodied in technoanatomy contributions, the doing of things right
or the failing to. The modern-day constraint that motivated the old
merge is then satisfied rather than violated: the internal hazard
remains high on the whole historical timeline's present because the
leaning of $\rho$ remains to be settled, which is consonant with the
scaling observation of \S\ref{gr:subsub:kardashev}, a
present-configuration scale-up collapsing, worse than ideal now yet
...
indicator's argument is being determined. The fluctuating-anatomy
corollary (Corollary~\ref{gr1:cor:recurrentanatomy}) is the bridge
case, fluctuation before settlement. The preceding passage's
...
Remark~\ref{gr1:rem:forwardforcing} states.

The formal debt this passage enters, at \textbf{[O]}, is the
$\rho$-dynamics window theorem it presupposes: conditions on the
settlement time and on the pre-settlement fluctuation under which
the frozen-$\rho$ analysis survives as the limit, the hazard
accumulated during the unsettled window remaining finite under
stated tail conditions so that the theorem's conditional structure
reads on $\rho_\infty$ unchanged. This is the best well-idealised
model the author can presently state, and it is entered as design
rather than result.
...
changes, because the ceiling bounds certification and never bounded
credence. Locally the two measures are equivalent, so every finite
...
Proposition~\ref{gc:prop:flukes} lives after its restatement. The
...
(Remark~\ref{gc:rem:sharpshooter}), the clustering texture of
...
that Remark~\ref{gr:rem:ceilingscope} already records. Two provisos
...
Remark~\ref{gr:rem:ceilingscope} left them. And the regime itself is
...
\eqref{sh:eq:G} its drift depends on $\kappa$, hence on both coordinates, so the comparison is a statement about the \emph{form} of the drift in
...
survival promotes the credit from the recurrent to the transient class, the P\'olya $2\mathrm{D}\to3\mathrm{D}$ transition, with no literal
...
\paragraph{The verdict.}\label{sh:rem:verdict}
...
\paragraph{The same refinement the companion performs, in the path
...
notes it. The companion derives classical survivorship bias as the Accordance
...
\begin{remark}[What the hazard rate is a projection of; \textbf{[$\Phi$]} on the companions]
...
did not occur. The over-idealised model the author proposes treats
...
than in a paper of its own. \emph{The two-condition theorem} (\ref{gr1:thm:twocond}) is the paper's
...
there does not yet reach it. \emph{The trapped-regime theorem} (\ref{gr1:thm:trapped}) does the work
...
ever have. \emph{The gauge results} of \S\ref{gr1:sub:coords} recur in a different
...
firmly as against its targets. \emph{The schedule channel} (\S\ref{sch:sub:schedule}) is consumed in
...
along which the relevant quantity, total hazard-bearing organization, varies. ``Civilization'' fails backward for the same reason and
...
\paragraph{What this already costs Kardashev.}\label{gr:rem:kardvocab}
...
a civilization, since ``civilization'' carries semantic content, agency, society, purpose, that the analysis is supposed to derive
...
division can do. \emph{First, the split is categorical and unchosen --- in the critical
...
regime-specific. \emph{Second, in the critical regime nothing occupies the Golden
...
Three consequences, and the middle one costs this section something. \emph{Trapped.} With adverse anatomy, viability is zero and life
...
epistemic ceiling is a statement about it. \emph{Viable.} With good anatomy and feedback, $\PP(T=\infty)>0$ and
...
fluctuates, and by Theorem~\ref{tc:thm:half} every biosphere, however well constituted, spends exactly half of logarithmic time
...
is not doing the work this subsection is meant to do. If the selection acted \emph{categorically on the structural parameter
and quantitatively on the state coordinate}, the asymmetry inverted, the account of why the two behave differently would be wrong, since
...
standing. And if the structural parameter showed selection \emph{as abrupt} as
...
be recorded rather than left for a reader to find. It is a claim about \emph{shape}, not magnitude. The framework does not
...
(Remark~\ref{gr:rem:betaalpha}, \cite{Tainter88}). Physical-provisioning
register: prepositioned prerequisites measured against a
habitability-conditioned base rate, with the census of systems as
the eventual instrument (Remark~\ref{rw:rem:preposition}). Transfer
...
a tumor does. Now consider how the two sides of this ledger scale with technostrength,
...
falsifiable in principle; none is currently testable. \emph{(a) Encountered biospheres are not extractive.} Not because
...
contribution. Gott's Copernican argument \cite{Got93} derives a predictive
...
free shape parameter, Weibull $k$, or a Pareto exponent $\alpha$, and study the consequences of its value. The disagreement is therefore
...
two halves turn out to partition the same problem. \emph{The same move, in opposite temporal directions.} The Accordance
...
threatening the paper. \emph{Collapse from declining returns.} Tainter's account of societal
...
uncertifiable in the direction that would have been consoling. The
live third term's interaction with this structure, amplification
without breach and a possible sharpening of the one-sidedness, is
priced at Remark~\ref{sch:rem:thirdceiling}, and the climb's
asymptotic phenomenology, the extreme-induction regime, at
Remark~\ref{sch:rem:inductionregime}.
\end{remark}

\paragraph{What now stands on the ceiling.}\label{gr:rem:ceilingconsumers}
...
\paragraph{What this costs each party.}\label{gr:rem:evenhanded}
...
encounter. Against \emph{this section}: no finite observation could establish that
...
\item \emph{The Extraction Hypothesis} ($\Phi3$), \textbf{conjecture-grade}, argued from boundary cost but not derived,
...
residual life, the $s/4\varepsilon$ criterion, the measured truncation, inherits the tags of \S\ref{gr1:sec}, in
...
\paragraph{What the stipulation does and does not buy.}\label{gc:rem:retrocaution}
...
forbidden ones, \emph{aims}, \emph{seeks}, \emph{progresses toward}, are as forbidden here as anywhere else in this work.
...
\paragraph{The exceedance form is the multiplicity guard.}\label{gc:rem:noisefloor}
...
strong speaking ability is never called on. Living the one I am in, where I have it, would be a decided fluke in that set. So, as an
...
Two things about the example, and the second is the important one. \emph{What it displays.} It is the retrocausal accordance principle
...
grade, $\kappa$-fold failures at probability at most $1/(1+\kappa)$, and one instance neither confirms nor refutes a tendency. The
...
\paragraph{Two directions, one of them deductive.}\label{ra:rem:twodirs}
...
sides, and conflating them is the error most worth guarding against here. Golden Optimation runs \emph{from} the conditioning target \emph{to} the
...
principles, the past-cone shadow of a future-cone conditioning, which is not a smaller answer than the one sought but a different and
prior one.
...
\paragraph{What the draw costs, and the bracket in both directions.}\label{ra:rem:drawcost}
...
the question, an interrogation-indexed reading of the observer tilt, and such situations are concentrated exactly where the questioner
is. The ratio deflates toward order one, the bound
\eqref{gc:eq:alphabound} goes slack, and the filter selects nothing. \emph{Upward.} Weight instead by observer-situations across the whole
...
So the ansatz is not a convenience chosen to make the argument work, one natural alternative sharpens it and the other dissolves it, and
...
\paragraph{What the bound is evidence for, and what it is not.}\label{gc:rem:alphareading}
...
elsewhere and would be inconsistent if it did not police it here. The companion's extraction is explicit that the global birth-rank
...
\paragraph{What this argument adds to the reductio.}\label{ra:rem:spine}
...
\paragraph{Golden Point Nexic Sorting, stated and deferred.}\label{ra:rem:sorting}
...
after, with $\PP(\mathcal G\mid M)\approx\PP(\mathcal G\mid A_2)$, reduces the cell ratio to
...
\paragraph{The exceedance is cheap, and the trade is worth stating.}\label{ra:rem:investkappa}
...
strength actually lies. Part~(i) is an equivalence and buys nothing on its own: it relates two
...
coin-flip's worth of confidence and should be told so. The reading this licenses is the one that gives the section its name.
...
given improbability must buy for the comparison to come out as observed, which is a different object from a stock, and the distinction is
exactly the one that keeps the economic vocabulary honest.
\end{remark}

\paragraph{Where this structure has already appeared.}\label{ra:rem:investinstances}
...
spent on. \emph{And the collapse.} \S\ref{gc:conv:collapse}'s condition
...
\paragraph{The exhaustiveness cost, again.}\label{ra:rem:investcost}
...
between this work and the companion by one level. By Remark~\ref{sch:rem:asymptote}, the only conditioning under which the
...
economic vocabulary of \S\ref{gs:sec} was built to carry it, Aurutance's two terms are the cost and the necessity, and the
...
honest. \emph{The evidence is the same object \S\ref{gs:sec} computes.} $\hm$ is
...
(\S\ref{gs:sub:limit}). \emph{And that map is what stops the procedure being unfalsifiable.}
...
\paragraph{Reading the law, and the coordinate it must be read in.}\label{gc:rem:lindy}
...
parametrization. \emph{Credence must be tracked in log-odds.} Posterior odds are
...
own foundation declines the inference it instantiates.} The Accordance Principle binds attribute by attribute, and its extraction
...
only. The evidence flux $\tfrac{d}{dt}\Lc_t$ is zero. \emph{Weak.} The forward cone is weighted by Golden-Point probability, so
...
positive and maximal. The discriminating quantity is therefore the flux, and it orders the
...
predictions of \S\ref{gs:sec}. The ceiling governs throughout, since
...
entitled to ask what follows for conduct, and the answer is not What the material is for, then, is bounded and worth stating: candidate
...
questions. \emph{The operation, and its licence.} Under the Golden target,
...
And the form chains. If $A$ is $G$-required and $A$ presupposes $B$, $\PP(A\mid\neg B)\approx0$, with $\neg B$ not itself
...
strengths. \emph{The framework's own answer, which is a theorem.} The inference
...
\paragraph{Which step to attack.}\label{gc:rem:actingcost}
The argument's steps are not equally exposed, and a reader deciding
where to push should be told which is which. Steps 2 and 4 are definitional or immediate. Step 6 is a theorem ---
...
ordering}, and an agent who holds values the ordering does not encode, who cares about suffering in a departed world, say, is not
...
reductio. \emph{The objection.} The fences say that whether one's own contribution
...
\paragraph{What the soft-killing statement costs, and what discharges part of it.}\label{gs:rem:hT}
...
\paragraph{The rule's neighbors: maxipok, ergodicity, ruin.}\label{gs:rem:ruleneighbors}
...
history occupies \cite{Peters19}. The kinship is real and structural, Aurutance is a log-ratio, additive across independent comparisons,
...
\emph{Ruin problems.} The precautionary literature's structural core, ruin is an absorbing barrier, so cost--benefit reasoning that
...
remaining mortal exactly as \S\ref{ac:sub:question} prices them.
...
the misfiling that \S\ref{gs:sub:notreligion} exists to refuse; the
...
\S\ref{ac:sub:question}; this subsection confines itself to stakes. The preceding leaves a gap that arithmetic alone will not close. If both
...
its conditions. \emph{Nature is the biosphere.} The unit \S\ref{gc:sub:vocab} adopts for
...
Golden Point is the only route by which the Departure Point is avoided, is \S\ref{ac:sub:imperative}'s, where it rests on the complementarity of
...
inflict hazard on one another and reorganize under a single principle, and reports how far that apparatus gets on its own. It does not yet
...
\S\ref{ad:sub:experiments}. \emph{Removal of a common enemy.} When the Welsh drove back the
...
\paragraph{The load-bearing departure.}\label{ad:rem:departure}
...
\paragraph{Disjoint ground states (\textbf{[P]}).}\label{ad:rem:disjoint}
...
\paragraph{The budget is not optional.}\label{ad:rem:apathy}
...
\paragraph{Reciprocity is not emergent.}\label{ad:rem:recip}
...
at the all-allied state, crushes it, since near any configuration of interest it then varies over
...
distributes the residual --- at $n=4$ the optimum is $(-1,-1,-1,-1)$. \emph{$n\equiv3\pmod4$: the imbalance concentrates.} Here individual
...
\paragraph{The scapegoat, and what it does not license.}\label{ad:rem:scapegoat}
...
\paragraph{Where factions appear.}\label{ad:rem:factions}
...
\paragraph{Fragmentation, not repolarization --- and no flips.}\label{ad:rem:welshreading}
Two features distinguish this from what either neighboring theory would
predict, and the second is a limitation. Balance theory predicts repolarization into two fresh camps, since two
...
historical record on the Welsh after the Anglo-Saxon reverse, increased factional subdivision without descent into general war, is
...
\paragraph{Incomplete unification, and an asymmetry.}\label{ad:rem:aliensreading}
...
qualified prediction than the verbal argument yields. \emph{Arrival and removal are not symmetric.} Adding a threat produced
...
\paragraph{What is missing, stated exactly.}\label{ad:rem:missing}
...
\paragraph{Two warnings.}\label{sh:rem:warnings}
...
\paragraph{Slogan.}\label{sh:rem:slogan}
...
\paragraph{Consistency with the scale-function formula.}\label{sh:rem:crosscheck}
...
\paragraph{Sharpness is a small-noise limit; general principle.}\label{sh:rem:FW}
...
\paragraph{Exponents measure smoothness, not recurrence class.}\label{sh:rem:smoothness}
...
\paragraph{The background is a partition function.}\label{tc:rem:partition}
...
\paragraph{Our two mechanisms are the two phases.}\label{tc:rem:twophases}
...
\paragraph{A headwind.}\label{tc:rem:headwind}
...
\paragraph{Void as mutedness.}\label{tc:rem:mutedness}
...
\paragraph{Why no capacity parameter is needed.}\label{tc:rem:nocapacity}
...
slopes, so nothing structural is lost. \emph{(b)} The gain range is $(0,\rho)$ and not $[1,\rho]$. The crest
...
\paragraph{Reading of the gain.}\label{tc:rem:gainreading}
...
\paragraph{This repairs revision~1's defect.}\label{tc:rem:repair}
...
\paragraph{What was given up.}\label{tc:rem:givenup}
...
\paragraph{Softening and re-hardening do not cancel each other ---
...
\paragraph{The Ansatz gap narrows.}\label{tc:rem:gapnarrows}
...
\paragraph{Grace, but not shelter.}\label{tc:rem:notshelter}
...
\paragraph{The endpoints are ray limits, not coordinate limits.}\label{sh:rem:raylimit}
...
\paragraph{Notable structures at Level 2.}\label{sh:rem:sheet:L2}
...
\paragraph{The interpolation as a one-parameter family.}\label{sh:rem:sheet:family}
...
\paragraph{Survivorship coupling is collider bias in continuous time.}\label{sh:rem:collider}
...
Four tests specific to this part, additional to the driftless case's. \emph{(i) Freezing.} Simulate $B_t$ for $n\in\{10^{3},10^{4},10^{5}\}$
...
after, the frozen phase being non-self-averaging. \emph{(ii) The two coordinates under conditioning.} Verify the sign
...
falsifiable claim. \emph{(iv) The gain profile.} Compute $\Psi'$ from
