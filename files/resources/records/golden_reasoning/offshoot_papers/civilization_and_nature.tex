...
argued away. And a consequence for alignment: the Golden Point supplies
a biospheric viability constraint external to any current party's
preferences, a necessary condition an aligned system must not violate,
which is not the whole objective, and which finite observation of a
trajectory cannot certify. The paper owns its premise, names its targets
...
which the literature has treated as central for a decade; it supplies a
constraint external to any party's preferences, a biospheric viability
condition that an aligned system must not violate whatever else it
optimizes, and it says plainly that survival ranks none of the things
two viable policies can differ in.
...
by none of them; it does not use the theorem companion directly. Its
references to the Miracleism companion \cite{Miracleism} are forward
pointers to downstream discussion only and carry no weight here, so
that the published order runs from this paper to that one and not
back. Nothing in this paper is offered as support for the Route World
...
about it, and it says it as a constraint. An adequate alignment scheme
must at minimum preserve the viability region of the two-by-two picture
of Section~\ref{sec:table}: whatever else a system optimizes, it must
not drive the biosphere into the cells where viability is zero, by
removing the muffling capacity or by turning the response sign adverse.
The Golden Point supplies that constraint, and supplies it externally
to any current party's preferences, which is what the alignment
literature's inventory of instructions, intentions, revealed and ideal
preferences, interests, and values does not \cite{Gabriel20}. It
determines nothing further. Two viable policies may differ arbitrarily
...
the Miracleism companion's subject and not this paper's
\cite{Miracleism}.
...
is small, which holds when $r$ is small against $B+\Gamma$, the pinned
hazard being the clock that recovery must beat; it is a statement about
a ratio of rates and not about the passage of time, and it is entered
at that grade, with $\alpha$ and $q$ unestimated \textbf{[O]}.\end{remark}
...
defensible result for an indefensible one. The Miracleism companion \cite{Miracleism} makes a
...
the alignment constraint: a demonstration that the constraint is
selected by some party's preferences after all; against the
certification claim, a finite trajectory shown to separate the two laws
...
\bibitem{Miracleism} A.~Benander, \emph{Miracleism: exotic structure and meaning after scientific disenchantment}, working draft, 2026.
 A.~Benander, \emph{The Route World: a Golden-reasoning hypothesis of biospheric natural history}, working draft, 2026.
