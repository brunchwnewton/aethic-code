...
exponent $\rho$. The paper's claim in one sentence: whether the
total hazard ever accrued stays finite is decided by the sign of the
response exponent and not by the magnitude of the accumulated
capacity, and no event of bounded duration on a surviving path
...
\emph{The claim.} Whether the total hazard ever accrued stays finite
is decided by the sign of the response exponent and not by the
magnitude of the accumulated capacity
(Theorem~\ref{thm:twocond}), and no event of bounded duration on a
...
$\varsigma_b(x)$ attenuating the background. \emph{Escape factor}:
the driver's probability of escaping to $+\infty$, the second factor of
Theorem~\ref{thm:twocond}. \emph{Hard wall / soft wall}: killing at
...
$|\rho|>1$ there is a unique interior critical point, a minimum, at
(writing $A:=Be^{b}$, $|\rho|=-\rho$, in the regime $x_\ast\gtrsim b$)
\begin{equation}\label{eq:xstar}
x_\ast=\frac{\log\bigl(A/(|\rho|\Gamma)\bigr)}{1+|\rho|},
\qquad
...
Ansatz~\ref{ans:soft} $\log\PP(T>t)=-\tfrac14\log t+O(1)$. The
derivation in the master manuscript \cite{GoldenMaster} yields the
more specific form
...
[C/A] (regularly varying tails, $\PP(T>t)\sim L(t)t^{-\alpha}$; the
critical case $\alpha=1/4$): the pure Doob limit of
Theorem~\ref{thm:Q}, locally absolutely continuous, globally singular,
its kinematic factor \eqref{eq:rate}. \emph{(III) Concentration} [S]
...
numbers for the rare, derived in the master manuscript
\cite{GoldenMaster} and cited here. Modes (II) and (III) share $\PP(\Imm)=0$ and are separated by
...
The claim stated at the outset stands as proved: viability is decided
by the sign of the response exponent and not by the magnitude of the
accumulated capacity, and no event of bounded duration on a surviving
...
the escape factor and is silent on the sign that multiplies it, so
