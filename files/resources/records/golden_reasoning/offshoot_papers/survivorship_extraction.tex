...
\date{September 2026 --- Working Draft, rev.~7}
...
a piecewise driver-viability functional --- the classical
scale-function escape probability for a transient driver, the
stationary-mean criterion in the positive-recurrent case, where the
driver does not escape ---
two logically distinct necessary conditions, with a closed-form Gaussian
...
survivors, globally singular, with a universal kinematic factor whose
linear-response coefficient is the tail index $1/4$, and classified
against exponential-tail regimes into three modes. Monte Carlo
experiments with exact pair simulation support the $t^{-1/4}$ exponent
...
\emph{technoanatomy}, in the downstream mechanism companion; here they
are the muffling
...
survivorship over deep time, is developed in the downstream companions of this branch, on which
nothing above depends and none of which is cited here.
