...
\begin{theorem}[Locally ours, globally elsewhere; \textbf{[P]} for the
hard-wall law, \textbf{[C]} for the soft-wall law, in \cite{SurvA}]
\label{thm:singular}
...
For a predeclared record statistic $E\in\Fc_s$, and with both laws
conditioned on the common baseline $C$ of Remark~\ref{rem:survnull}
wherever an observer's record is compared, the Bayes factor between the
conditioned and unconditioned laws is
\begin{equation}\label{eq:bf}
\mathrm{BF}_{Q:\PP}(E)=\frac{Q(E\mid C)}{\PP(E\mid C)},
\end{equation}
which reads $Q(E)/\PP(E)$ when no baseline is shared;
...
\begin{theorem}[No bounded observation certifies the law; grade as
Theorem~\ref{thm:singular}]\label{thm:ceiling}
