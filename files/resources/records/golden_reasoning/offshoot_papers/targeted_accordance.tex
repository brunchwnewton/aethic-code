...
consequences, are developed in a separate companion \cite{RouteWorld}, so
that the theorems here and the hypothesis there carry their different
grades separately. Drafting collaboration: Anthropic Claude (Opus~4.x,
...
future-conditioned law is made here; that is the companion's hypothesis
\cite{RouteWorld}, and it is carried there at a hypothesis's grade. No
...
$Q$-process is the limit $Q:=\lim_{t\to\infty}Q_t$ on each $\Fc_s$, which
exists and is the Doob transform by the persistence-harmonic function
$\hm$: $dQ/d\PP|_{\Fc_s}=\one_{\{T>s\}}\hm(X_s)/\hm(X_0)$ in the hard-wall
model, with the soft-wall version conditional on the ratio hypothesis the
companion states.
\end{definition}
...
\begin{theorem}[Horizon-indexed accordance]\label{thm:horizon}
...
\end{equation}
and on the full information $\Fc_s$ it is the likelihood ratio
$Z_s:=dQ|_{\Fc_s}/d\PP|_{\Fc_s}$, which in the survivorship model is the
Doob density $\one_{\{T>s\}}\hm(X_s)/\hm(X_0)$. If $E$ is required by the
...
\frac{dQ}{d\PP^{\mathrm{surv}}_s}\Big|_{\Fc_s}=\PP(T>s)\,\frac{\hm(X_s)}{\hm(X_0)},
\end{equation}
...
2026.
\bibitem{RouteWorld} A.~Benander, \emph{The Route World: a Golden-reasoning
hypothesis of biospheric natural history}, working draft, 2026.
