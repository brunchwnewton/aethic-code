...
The pieces now assemble. (A) and (B) remove the human route;
Proposition~\ref{gc:prop:disjoint} makes every alternative route an
Imperfection Magnitude Drop. So on any branch carrying no new muffling
agent, there is no route by which one re-arises, and by
Proposition~\ref{gc:prop:onefloor} the external channel acquires a
permanent floor: it will never again fall. One such channel suffices.
...
they mean to keep.%
\footnote{\textbf{VERIFY AT MERGE.} An earlier draft attributed the
Newcomb treatment to a companion manuscript under a title that does not
correspond to any existing work; the citation has been withdrawn and the
paragraph restated so that it rests on the structural parallel alone,
which a reader can assess directly. If a companion treatment of
Newcomb's problem exists in the corpus, attach it here; if none does,
the paragraph stands as written and claims no prior treatment.}
