...
Assume $x_0=\mu_0=0$. Normalise by the driftless case's own scaling exponents
...
(the driftless case's Lamperti argument), so Birkhoff gives density
$\PP(Z_0<0)=\tfrac12$ by symmetry.
\end{proof}

\begin{remark}[What the sharpening owes to the polar form]
\label{tc:rem:owes}
Theorem~\ref{tc:thm:half} genuinely sharpens the driftless case, whose
...
no terminal stage and no monotone ladder; the rejection of the
...
\emph{First, the split is categorical and unchosen.} $Q$ and $\PP$ are
\emph{mutually singular} on the infinite-horizon $\sigma$-algebra
(\S\ref{gr1:sec}): there is an event --- the credit tending to infinity ---
which has $Q$-probability one and $\PP$-probability zero. General and
Golden are therefore not two regions of one population separated by a
line someone drew. They are different measures, and the difference is
established by a theorem rather than a stipulation. Pre-Civilization
versus Civilization has no such status; it is a threshold in a
continuous variable, and which variable and which value are both
contested.

\emph{Second, nothing occupies the Golden category --- as an ideal.} By
the mode classification of \S\ref{gr1:sec}, the driftless case is
\emph{critical}: the survivor's law is approached but entered only at
infinity. Every actual biosphere is somewhere on the interpolation
...
nothing occupies (Definition~\ref{gr:def:gg}), and $\PP(T=\infty)=0$ ---
in this model nothing survives forever. What $Q$ supplies is a
...
Under the polar decomposition of \S\ref{gr1:sec}, the state of a biosphere
factors exactly and adaptedly as
$x_t=\sigma t^{3/2}R_t\cos\Theta_t$, where $R_t$ is the
\emph{technostrength} and $\Theta_t$ the \emph{technoanatomy}. Under
$\PP$, at each fixed time, $R$ is Rayleigh$(1/\sqrt3)$, $\Theta$ is
uniform, and the two are independent. \emph{Bad technoanatomy} is
$\cos\Theta<0$: the configuration in which the biosphere's own
organisation places its hazard above the background.
\end{definition}

Two results of \S\ref{gr1:sec} are worth restating in this vocabulary because
they are counterintuitive.

\emph{Technostrength is not cumulative.} $R$ is stationary in
logarithmic time and recurrent. It is not a stock that accumulates but a
magnitude that fluctuates. Any framework which treats the relevant
quantity as monotonically increasing has, at the outset, the wrong kind
of object.

\emph{Every biosphere is badly configured half the time.} By the exact
half theorem of \S\ref{gr1:sec}, the set $\{t:\cos\Theta_t<0\}$ has
logarithmic density exactly $\tfrac12$, for every parameter value and
every biosphere. Bad technoanatomy is not a defect distinguishing some
biospheres from others; it is a phase that all of them occupy half of
logarithmic time.
...
& General ($\PP$) & Golden ($Q$)\\
\midrule
$\PP(\text{bad technoanatomy})$ & $0.501$ & $0.0037$\\
median technostrength $R$ & $0.681$ & $1.267$\\
$\PP(|\Theta|<\pi/4)$ & $0.250$ & $0.855$\\
$\mathrm{Corr}(R,\cos\Theta)$ & $-0.002$ & $+0.001$\\
\bottomrule
\end{tabular}
\end{center}

\begin{remark}[Truncation versus rescaling]\label{gr:rem:truncresc}
The headline number is that bad technoanatomy is suppressed by a factor
of $137$. It is tempting --- and the present authors did initially
suppose --- that this shows conditioning acting on technoanatomy far
more strongly than on technostrength, which would make a
strength-indexed scale simply the wrong coordinate. Measurement refutes
this. The Kullback--Leibler divergences of the two conditioned marginals
from their unconditioned counterparts are
\[
\mathrm{KL}(Q_\Theta\,\|\,\PP_\Theta)=1.16\ \text{nats},
\qquad
\mathrm{KL}(Q_R\,\|\,\PP_R)=0.94\ \text{nats},
\]
a ratio of $1.23$: conditioning acts on the two coordinates with
comparable \emph{force}. The difference is one of \emph{kind}.
Technostrength is \textbf{rescaled} --- inflated by a factor $1.86$,
its distribution retaining roughly its shape. Technoanatomy is
\textbf{truncated} --- a half-space is very nearly deleted. The $137$ is
a tail statistic, not a summary.

This matters for what may honestly be concluded. One may not say that
survivorship cares about anatomy and not about strength. One may say
that it acts on strength quantitatively and on anatomy
\emph{categorically}, and that a one-dimensional index of magnitude
therefore cannot represent the categorical part of the selection at all,
whatever it does with the rest.
\end{remark}

\begin{remark}[The coordinates stay separable]\label{gr:rem:separable}
$R$ and $\Theta$ are independent under $\PP$ by construction. The
measurement above shows they remain almost uncorrelated under $Q$
($+0.001$). Conditioning on survival therefore does not fuse the two
notions: technostrength and technoanatomy remain separately inferable
even in the Golden ensemble. This is what licenses treating them as
distinct diagnostic questions rather than as two aspects of a single
``advancement''.
\end{remark}
...
\emph{(i) There is no terminal stage.} In the model, viability is
impossible: $\Lambda_\infty=\infty$ almost surely, for every parameter
value and every initial condition, in both the \S\ref{gr1:sec} and the
two-channel form (\S\ref{gr1:sec}). Death is certain. There is no absorbing
state of safety, no Type~III, no top. A scale whose highest category is
a destination misdescribes the geometry of the problem: the system is
null recurrent, not convergent. This is the strongest available
anti-Kardashev result and it does not depend on any premise of this
section.
...
The inference has the form: predatory origin $\Rightarrow$ predatory
maturity. Golden reasoning denies the implication, on the grounds that
it neglects the conditioning that must be applied before any encounter
can be contemplated. Any biosphere one could meet is old; any biosphere
...
against}, one needs the identification of predation with bad
...
of a locale and moving on --- is \emph{bad technoanatomy}: it places the
biosphere's hazard above background through its own internal structure,
and does so increasingly with technostrength.
\end{premise}
...
The domain over which extraction occurs grows with reach: it is a
volume-like quantity. The enforcement of the boundary is exercised
across an interface: it is a surface-like quantity. Volume outgrows
surface. Hence:
...
\noindent This is the serious objection and we do not refute it. It is
an empirical claim about the scaling of coordination costs, and it is
where $\Phi3$ is falsifiable. We note only that every known instance of
large-scale organisation --- organisms, ecosystems, institutions ---
exhibits the opposite scaling, and that the burden of the exception lies
with whoever claims it.
...
\noindent The charge would hold if $\Phi3$ were derived from the model.
It is not: the model says what happens to bad technoanatomy, and is
silent on what bad technoanatomy \emph{is}. The identification is
supplied from outside, by \S\ref{gr:subsub:boundary}, and could be
false. The division of labour is exactly the one this section's method
...
$\Phi3$ fails if boundary-maintenance cost is shown to scale
sublinearly in reach; if an extractive organisation at scale is
exhibited stable over timescales long compared with its own doubling
time; or if internal hazard can be made to \emph{decrease} with
technostrength, contradicting the trough channel's structure. The first
...
and the section's principal exposure. Its position in the argument is
...
known results on sums of random exponentials --- and the Lindy
conclusion of \S\ref{gr:sub:consequences} is established prior work
\cite{BC21,BG24,Kip20,Ord23}. What is claimed as new is the assembly
...
six costs are costs \emph{of}. What is unconditional is the selection
structure: that $Q$ and $\PP$ are mutually singular, that conditioning
truncates one coordinate and rescales the other, that no terminal stage
exists. What is priced is every sentence connecting that structure to
...
The categorical division is \emph{unchosen}: General and Golden are
mutually singular measures, so the split is established by a theorem
rather than by a threshold someone placed, which is what
Pre-Civilization/Civilization and every Kardashev boundary cannot say.
...
being chosen, since the two are mutually singular; but nothing occupies
...
not that the future goes well but merely that there is one. The state factors into technostrength and
technoanatomy, which conditioning treats differently in kind: strength
is rescaled by a factor $1.86$, anatomy is truncated, with bad
technoanatomy suppressed $137$-fold --- though the two divergences are
comparable in magnitude, $1.16$ against $0.94$ nats, so the honest claim
is about kind and not force. The Kardashev scale fails not because its
thresholds are arbitrary but because the model has no terminal stage,
its indexed quantity does not ratchet, and a magnitude index cannot
express a deleted half-space. The violent-aliens thesis fails at the
...
\subsubsection{Reduction to the driftless case}\label{tc:subsub:reduction}
