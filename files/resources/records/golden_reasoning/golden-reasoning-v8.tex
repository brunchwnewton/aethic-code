...
conditioned on the production of an observer. \emph{Optimation} names
that conditioned measure; the fluke content a record so conditioned
displays against $N_0$'s own rates is what the measure \emph{shows},
and this work uses the word for the measure and speaks of its
``fluke content'' when the display is meant \cite{Aethus, Nexus,
Ben26Found}. The \emph{Aetherian Nexus} is not a weighting but an
Aethus --- the one of which our own Aethus is a proper child --- and is
used in that sense throughout.
\end{convention}
