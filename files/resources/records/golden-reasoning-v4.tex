...
An \emph{Imperfection Magnitude Drop} is what results from complementing
a stated attribute correlated with the remainder of an Aethus: the
...
companion's grade (Remark~\ref{gc:rem:twolegs}). Two reasons, kept
...
outright, since some weight survives on branches reaching the Golden
Point by other routes; $\varepsilon$ is that weight, and
\S\ref{gc:subsub:grading} gives both its mechanism and its formal
statement. The reasons above bear on it in a specific way worth keeping
straight: they make the human route contribute \emph{nothing},
removing it rather than lengthening it, so whatever $\varepsilon$ is,
none of it comes from here. What makes $\varepsilon$ small rather than
merely positive is the displacement supplied by the High-Contrast Earth
Hypothesis, which the derivation below sets out and
Remark~\ref{gc:rem:twolegs} attributes.

\emph{The argument is the standard IMD derivation, run on (A) and (B).}
An Imperfection Magnitude Drop is a targeted perturbation away from
one's own Aethus, yielding a natural history whose success-intersection
is drastically minute but never quite zero
\cite[\S IMD]{Nexus}. Human extinction complements (A) and (B): the
last common ancestor is no longer a stated attribute available to be
run through, and the climatic-geographical setting has been cashed out.
Any Aethus continuing from that state therefore carries those
attributes in their complemented form.

Two established results then apply in their usual order, and keeping
them in that order is what makes the conclusion available. The
...
drastically minute, which is precisely what it is to be an Imperfection
Magnitude Drop.
...
happen, and a post-extinction biosphere has deep time in abundance. The
reply would land against a claim about rates. It does not land here,
because the displacement is a property of the \emph{perturbation} and
not of a waiting period: complementing a correlated attribute
re-randomizes the success-intersection wherever in time the
continuation is read, and elapsed time supplies no mechanism returning
it to the unperturbed value. This is also why no separate treatment of
``some other lineage'' is owed. What has been given, though, deduces
the drop without situating it, and those are separate tasks: the
deduction prices a perturbation, and what follows says which
perturbation is being priced.

A successor does not begin from nothing. Whatever lineage one nominates
--- the usual candidates are other primates, and the corvid and
cetacean lineages in which complex cognition has arisen independently
--- it exists now, at its present distance from the capacities at
issue, and the
question is whether that \emph{gap gets closed}. Ours was closed by a
particular route, and (A) and (B) name what closed it: a specific
ancestral bottleneck, and a specific climatic and geographical sequence
that drove the lineage into the setting where the capacities were
selected for. A successor needs its own analogues of both. It does not
inherit ours, which are spent, and there is no reason the corresponding
attributes of its own history should be favourably placed --- that is
exactly what having a different history means.

So the claim is not that a successor is impossible. It is that
\emph{the probability of the gap being closed is itself an Imperfection
Magnitude Drop}: the successor's Aethus is ours with (A) and (B)
complemented, it receives the same displacement by the same two
results, and the displacement is what an Imperfection Magnitude Drop
is. There is no second population of alternatives to survey, because
every alternative is a perturbation of ours --- and the perturbation is
priced by the machinery already in hand rather than by a fresh
argument.

The division of labour between the two presentations should be kept in
view, since neither substitutes for the other. The perturbation
argument carries the deduction and the naming of lineages adds nothing
to it: no weight rests on which animals are nominated, and the
conclusion would stand if none were. What the lineages supply is the
referent --- they say what the perturbed attributes belong to, and so
what the displacement is a displacement of. Deduction without referent
leaves ``every alternative is a perturbation of ours'' looking like a
definitional manoeuvre; referent without deduction leaves a story about
animals, which invites the answer that some animal or other will
manage it.
...
identification of (A) and (B) as the attributes perturbed; the
displacement is then supplied by the High-Contrast Earth Hypothesis and
its direction by the Accordance Principle, both imported at the
companion's grade \cite{Nexus, Ben26Optimal}. The substitution property
is what permits the same treatment when the objector's alternative
lineage is the subject, since a trait of that lineage may itself be
placed in the conditioning seat.
