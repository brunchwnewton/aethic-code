...
Some thirty million years ago a viral epidemic came near to
extinguishing the Catarrhine lineage. The virus presented
$\alpha$-gal, a carbohydrate our own cells also produced, so that no
immune response was available which did not attack the host. The
survivors were those carrying a mutation disabling \textit{GGTA1}, the
gene responsible for producing it --- a gene retained by every other
mammalian lineage, and lost in ours alone.
...
result. \emph{Falsifier:} a demonstration that deliberative and
evolutionary hazard-reduction differ in a respect the conditioning must
track --- that some feature of the record is explicable under one and
not the other. The agency objection is the natural candidate and is
answered above; a better one would consist in exhibiting such a feature
rather than in restating the intuition.
\end{remark}
...
develops very differently. Each removes the capacity:
...
almost surely. So for any Golden-relevant trait one names, perturbations
...
If for every Golden-relevant trait the perturbed histories systematically
...
and unlikely to survive reopening in the Aetherian Nexus. The totality of
...
weight claim is the load-bearing one and is asserted rather than
computed: that the counterfactual sum outweighs the Aetherian Nexus is
plausible on the grounds that removing improbable prerequisites raises
probability, but no bound is given here, and a demonstration that the
sum is comparable or smaller would remove the contradiction. Second, the
...
background runs unopposed. Destabilisation of the star, a sufficiently
...
So the choice is not between a damaged biosphere and an undamaged one.
...
\begin{remark}[The concession withdrawn: adverse anatomy does not
vindicate the position]\label{hr:rem:concession}
An earlier draft conceded that under $\rho<0$ the extinctionist is
closer to right, on the ground that $x\equiv0$ leaves hazard merely at
background while adverse anatomy drives it upward. The concession was
wrong on its own terms, and the correction matters because it changes
which option the argument is really between.
...
already been spent. A biosphere at $x\equiv0$ has not merely a low
