...
\bibitem[Ben26w]{WeightAlg} A.~Benander, \emph{The Weight Algebra of
...
Advancing requires $\Lambda_\infty<\infty$, which requires
$h^{\mathrm{tot}}_t\to0$, which --- the total being a \emph{sum} of
non-negative channels --- requires \emph{every} channel to vanish.
Hence a single channel possessing a permanent floor, one with no
remaining route by which it falls arbitrarily again, forecloses on its
own. One does not need the whole base hazard to have the property, and
in particular one does not need the internal channel to participate:
a permanent floor in the external channel alone places the state in
$\mu G$.
\end{proposition}

\begin{example}[Human extinction as an Imperfection Magnitude Drop;
\textbf{[$\Phi$]}]\label{gc:ex:humanIMD}
Human extinction is an Imperfection Magnitude Drop, and for reasons
that are structural rather than sentimental. The cascade to human
evolution ran through the human--chimpanzee last common ancestor,
which no longer exists to be run through again; and the specific
sequence of climatic and geographical events that brought that lineage
down to the grasslands has been cashed out, the world now standing at a
different stage. The pathway is therefore not merely improbable on
repetition --- it is \emph{unrepeatable}, its preconditions spent.

The consequence combines with
Proposition~\ref{gc:prop:onefloor}. With no muffling agent and no
route by which one re-arises, the external channel acquires a permanent
floor: it will never again fall. One such channel suffices, so on every
branch carrying no new muffling agent, human extinction is a Departure
Point outright --- in the modal sense of
Definition~\ref{gc:def:departure} and not merely a step toward one.
Across the full Aethic weight it is an $\varepsilon$-Departure Point in
the sense of Definition~\ref{gc:def:epsdep}, with $\varepsilon$ the
weight of the recovery branch; the reasons above are what make that
$\varepsilon$ small, since they remove the route rather than merely
lengthening it (Remark~\ref{gc:rem:boltzmann}).

\emph{Why this is not a probability claim, and why that matters.} The
distinctive move is the conversion of an improbability into an
\emph{unavailability}. ``Re-evolving humans is unlikely'' invites the
reply that deep time makes unlikely things happen --- and deep time is
exactly what a post-extinction biosphere has in abundance, so on
probability grounds the reply is unanswerable. Neither (A) nor (B)
operates on probability. A last common ancestor is a particular
historical lineage, not a member of a repeatable class; once gone it is
not drawn again. A climatic-geographical sequence that has run is a
spent event, not a low-rate one waiting to re-fire. Neither is unlikely
per unit time. Both are \emph{absent from the option set}, and elapsed
time does not restore them.

\begin{remark}[Two legs, and their separate falsifiers]
\label{gc:rem:twolegs}
This closes an objection that would otherwise stand against
\S\ref{hr:sec}: that a successor lineage might re-evolve the muffling
capacity, so that the accumulation is inherited rather than forfeited.
But the closure has two legs, and they should not be run together,
because they are established by different evidence and fail to
different attacks.

\emph{Leg one: the realised route is unrepeatable.} This is (A) and (B)
above, and it is what defeats the deep-time reply. It establishes that
\emph{the human route} is gone --- and no more than that.

\emph{Leg two: no alternative route carries comparable weight.} An
objector may grant leg one entire and continue: humans will not return,
but some other lineage might find another way. What answers this is not
(A) or (B) but the High-Contrast Earth Hypothesis together with the
substitution property of the Accordance Principle
\cite{Nexus, Ben26Optimal}: alternative routes are not merely
different but \emph{multiplicatively distant}, and the substitution
property runs the argument at any rung, so a trait of the alternative
lineage may itself be placed in the conditioning seat. Leg two is what
keeps $\varepsilon$ small rather than merely nonzero.

The division matters for what can be attacked with what. Leg one fails
if the last common ancestor or the climatic sequence turns out
reproducible in the relevant sense --- a claim about historical
individuals and spent events. Leg two fails if some alternative lineage
is shown to carry Golden probability comparable to ours --- a claim
about contrast ratios. Run together, the weaker leg's evidence appears
to support the stronger claim, and a critic may attack ``no agent can
arise'' using only what was offered for ``humans cannot return.'' Leg
one is original to this work; leg two is imported and carries the
companion's grade.
\end{remark}
\end{example}
...
recovery to zero. Some minute weight remains on branches in which the
Golden Point is somehow reattained, and this sits in apparent tension
with a Departure Point defined by a strict modal quantifier: if
\emph{some} continuation advances, however faintly, then not every
continuation fails, and the label should not apply.

\emph{One resolution is unavailable, and saying why fixes the scale of
the problem.} The companion's visibility semantics holds that carriers
at negligible weight are not invalidated --- no tag flips --- but fail
to be visible to the consistency postulate's recursion, and so cannot
come to the aid of preventing invalidity \cite{WeightAlg}. That
mechanism does not apply here. The negligibility relation $\ll$ is
non-Archimedean, stratifying zero weight, null-set presence, and
null-set presence within a null set; at the real-weights base case the
measure-zero class is exactly $\{0\}$. A recovery probability of
$10^{-40}$ is not negligible in that sense but an ordinary positive
real, hence visible, hence a witness in $S(x)$ --- and the state is
therefore \emph{not} labeled invalid. Small is not null, and the
apparatus is careful about the difference.

\emph{The correct reading keeps the superposition.} The post-drop state
is a genuine weighted combination of a departed branch and a recovery
branch, both valid, in the proportion the physics dictates; nothing is
rounded and nothing is deleted. What follows is that the Departure
Point label, taken strictly, does not attach --- and that the honest
statement is graded rather than binary.
...
displaces the other, and the temptation to make the cascade itself
graded should be resisted until someone works out what a fixed point of
an $M$-valued doom operator is --- which is open \textbf{[O]} and is
the natural next question this remark raises.
\end{remark}
\end{remark}
