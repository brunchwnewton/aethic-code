...
merely positive is a separate matter, and is leg two of
Remark~\ref{gc:rem:twolegs}.
...
recovery to zero, and it is worth being exact about why not, since the
mechanism is not the one the phrase ``recovery'' first suggests. What
remains is not weight on branches in which the \emph{same} route is run
again: that route is foreclosed by (A) and (B) of
Example~\ref{gc:ex:humanIMD}, and foreclosed by unavailability rather
than by improbability, so no weight sits there at all. What remains is
weight on branches along which some \emph{other} series of events ---
itself passing through Imperfection Magnitude Drops, as any long
natural-historical route must --- arrives at the Golden Point by a
different path entirely. Such routes exist. The High-Contrast Earth
Hypothesis is what makes them minute: alternatives are not merely
different from the realised route but multiplicatively distant from it
in Golden probability, so their total weight is small \cite{Nexus,
Ben26Optimal}.

The two contributions to the recovery weight are therefore of different
kinds, and conflating them costs the argument its structure. The human
route contributes \emph{nothing}, because it is absent from the option
set. Alternative routes contribute \emph{something small}, because they
are present but multiplicatively remote. It is the second alone that
makes $\varepsilon$ positive --- and this is exactly the division of
labour that Remark~\ref{gc:rem:twolegs} draws between its two legs.
