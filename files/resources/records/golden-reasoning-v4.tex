...
The \emph{Departure Point} is the complement of the Golden Point: the
event at which a biosphere departs from advancement once and for all.
The primitive is \emph{failure to advance past the point}, and the
ordering matters. Biological death is not the definition with
advancement as a gloss; it is one instance of the definition. A
biosphere may fail to advance by dying, but equally by persisting
without ever again approaching the Golden Point, or by regressing
irreversibly --- and all three are the same event under this
description, differing only in how they are realised.

What is being extracted is the \emph{phenomenology of failure}, taken
apart from the particular mechanism of biosphere-death in which it is
most visible. Once extracted it applies wherever there is a trajectory
to fail at, which is why the notion carries across the scales that
Principle~\ref{gc:prin:continuity} needs it to.
...
\emph{of the Departure Point}, not of death; the muffling exponent $x$
is accumulated against that; and every Imperfection Magnitude Drop is a
movement toward it. And the survival functional
$\Lambda_\infty=\infty$ is the analytic form of failure-to-advance
rather than a statement that the biosphere expires --- which is why the
trapped regime of Proposition~\ref{gr1:prop:trichotomy}(iii) counts as
a Departure Point outcome even where a biosphere halts at $x_\ast$ and
persists there indefinitely. Halting is not surviving; it is failing to
advance at a fixed rate of hazard.
...
(Definition~\ref{gc:def:departure}), not of biological death, and
$h^{\mathrm{tot}}$ is that event's hazard. Biological death is one way
of realising it; persisting indefinitely without further approach to
the Golden Point is another, and the analysis does not distinguish
them, because for the purposes of advancement they are the same
outcome. Where the classical results imported below speak of survival
...
probability that the biosphere \emph{advances} --- that it does not
depart. The two-condition theorem below therefore does not say what it
takes to avoid dying; it says what it takes to keep going, and the two
factors are the two things keeping going requires. That reading is what
makes the trapped regime intelligible: a biosphere halting at $x_\ast$
has not died, and has still departed.
\end{remark}
...
anyway. Under the reading on which the Departure Point is
\emph{failure to advance}, no such demonstration is needed: $x\equiv0$
\emph{is} failure to advance, immediately and by construction, whether
or not anything dies for a very long time afterwards. A biosphere left
undisturbed has not been preserved; it has been placed in the one
configuration from which the Golden Point is unreachable with
probability one. The extinctionist is not choosing a world that
