...
\bibitem{koide1983new} Y.~Koide, A fermion--boson composite model of quarks and leptons, \emph{Phys. Lett. B} 120 (1983) 161--165.

\bibitem{Brannen2006TheLM} C.~A.~Brannen, The lepton masses, preprint (2006).
...
an external background muffled by accumulated capacity, plus an internal
term the same capacity feeds --- and place the noise on the negative
logarithmic derivative of the hazard, one integration further out than
is standard. Three results follow. Survival requires a \emph{sign}
condition on how hazard responds to capacity, which we call
\emph{technoanatomy}: where that sign is adverse, viability is
identically zero for every parameter value and every driver, so no
amount of capacity helps. Where it is favourable, viability is the
product of that indicator with a transience factor, and the two
conditions are independent. And conditioning on survival is a Doob
$h$-transform rather than a threshold, so the distinction between a
biosphere considered unconditionally and one considered under
survivorship is categorical rather than a matter of degree --- with the
consequence that no observation of bounded duration can establish which
one is in.

The model's conclusions cut against two familiar positions on the same
grounds. Programmes that measure civilisational progress by capacity
alone are indexing the multiplicand and not the indicator; programmes
that would withdraw capacity leave the background unmuffled and reach
viability zero by the other route. We derive both from the two-condition
structure rather than arguing them separately.

The model is one of three parts, and the abstract should say so. The
second is a conditioning argument: that the natural-historical record we
hold is the past-light-cone restriction of a conditioning on an event in
our \emph{future} light cone --- indefinite survival of the biosphere ---
and that the companion program's observer-conditioning is exactly what
that stronger conditioning looks like restricted to the pre-technological
record. The argument is carried at a graded strength, its two runs are
ordered by how contestable their premise is, and its strong forward
reading, the Route World, is developed as the one hypothesis in the
family with a phenomenology and a discriminating quantity, though no
decisive test --- the last by theorem.

The third part is a set of positions, each conditional on a single
adoptable premise and each fenced: a grounding for the paper's normative
vocabulary, a decision rule whose limit form is a ratio of
persistence-harmonic values, and an epistemic ceiling turned against this
work's own conclusions as firmly as against its targets. The dependency
between the parts runs one way: the results and the argument consume
nothing from the positions, and a reader unpersuaded by the positions is
left holding the first two intact.
\end{abstract}
...
on a reading with no resource to discharge the fluke.
\S\ref{ra:sub:finale}
...
decides whether a civilisation lasts. This work argues that it is the
wrong quantity, and that the right one is a sign rather than a magnitude.

Consider a system facing hazard from two directions: an external
background it can learn to muffle, and an internal channel that its own
capacity feeds. Accumulating capacity lowers the first and does something
to the second, and \emph{what} it does to the second is a structural
property of the system rather than a matter of how much capacity it has.
Call that property technoanatomy, write it $\rho$, and the central result
is that its sign is decisive: adverse $\rho$ makes indefinite survival
impossible outright --- not unlikely, impossible, for every parameter
value and every history --- while favourable $\rho$ makes it possible
without making it likely, since a second and independent condition must
also hold.

Two consequences follow immediately and both cut against widely held
positions. A programme that measures progress by energy captured, or by
capability acquired, is measuring a quantity that cannot distinguish the
two cases; and a programme that would withdraw capacity leaves the
background unmuffled and arrives at the same zero from the other side.
These are not two objections but one, and the paper derives them
together.
...
One reconciliation is owed at the outset, because it is the objection a
reader will arrive holding. This paper builds a decision rule and an
ethics on a conditional distribution, and a conditional distribution
entails no obligation. The reconciliation is a division of labour between
two premises. $\Phi2$ (Premise~\ref{gr:phi2}) governs what the
\emph{apparatus outputs}: distributions, from which no duty follows.
$\Phi6$ (Premise~\ref{ac:phi6}) governs what an \emph{agent does with a
choice the apparatus never makes} --- which grounding to occupy, given
that some grounding is occupied whether or not anything is said. The two
operate at different levels and do not meet.

The consequence is that everything normative in this work is conditional
on $\Phi6$, which is offered and priced and may be declined. An agent who
declines it contradicts nothing proved here.
\S\ref{ac:sub:countermeasure} states the split in four checkable steps,
and \S\ref{gs:sub:fence} restates it where the decision rule might
otherwise be over-read.

One structural fact should be stated at the outset, because it is the
protection the mathematics needs from the rest of the document. This work
contains three kinds of thing. \emph{Results}: the hazard model of
\S\ref{gr1:sec} and its theorems, and the survivorship transform of
\S\ref{gr:sec}. \emph{A graded argument}: the conditioning principle of
\S\ref{gc:sec}, carried at \textbf{[S]}, with the extrapolation
hypotheses at \textbf{[O]}. And \emph{positions}: the grounding of
\S\ref{ac:sec}, the decision rule of \S\ref{gs:sec}, the ethics of
\S\ref{gs:sub:ethics}, and the placement of \S\ref{hr:sec} against
extinctionism and techno-optimism, all carried at \textbf{[$\Phi$]} and
all conditional on the one adoptable premise.

\emph{The dependency runs one way.} The results consume nothing from the
positions; the graded argument consumes nothing from the positions; the
positions consume both. A reader unpersuaded by the positions is left
holding the results and the argument intact, and nothing in the positions
can be used against them. A reader who finds the positions objectionable
--- who reads a paper on survivorship conditioning taking a stance on
longtermism as advocacy --- should read the results and the argument
first and the positions never, and will have read everything this work
proves.
...
carried at \textbf{[O]} throughout and can be read after \S\ref{gc:sec}
...
\begin{convention}[Weighting schemes]\label{gc:conv:weightings}
An \emph{Aethus} is an information state, deductively closed and indexed
to a bearer; its \emph{proper children} are its refinements. A
\emph{Nexus} is a weighting scheme over Aethae. The \emph{Cosmic Nexus}
$N_0$ is the unconditioned one; the \emph{Nexus of Optimation} is $N_0$
conditioned on observer-production. \emph{Optimation} is the fluke
content a record conditioned on observerhood displays against $N_0$'s own
rates \cite{Aethus, Nexus}.
% AUTHOR-CHECK (new on this branch): this convention formerly named the
% observer-conditioned weighting "the Aetherian Nexus", while
% gc:conv:vocab below and every body use (gc:sub:humanhistory,
% gc:sub:naturalhistory, ac:sec) treat the Aetherian Nexus as an AETHUS
% --- "our own Aethus is a proper child of the Aetherian Nexus", "the
% Aetherian Nexus is in fact our parent" --- and name the observer-
% conditioned weighting "the Nexus of Optimation".  Renamed here to
% match.  Still unreconciled: this convention glosses Optimation as a
% displayed fluke CONTENT, gc:conv:vocab as a conditioned MEASURE;
% the companion (Ben26Found) conditions on "the production of an
% observer" under the existence reading.  Pick one gloss.
\end{convention}

\begin{convention}[Perturbation and contrast]\label{gc:conv:imd}
An \emph{Imperfection Magnitude Drop} is a perturbation that suppresses
an Aethic weight drastically without foreclosing it --- the graded
counterpart of departure, and the object
Definition~\ref{gc:def:epsdep} refines. The \emph{High-Contrast Earth
Hypothesis} holds that each unit of perturbation depth suppresses the
success-intersection by an effective contrast $c_{\mathrm{eff}}$ in
orders of magnitude, with the sign of the contrast deliberately withheld;
the \emph{Strong Accordance Principle} supplies the direction separately,
and is exactly the statement $c>k$ where $k$ is the prior-odds toll
\cite{Ben26Optimal}.
\end{convention}
...
stated where it is used rather than here; the four items above are the
ones this section leans on repeatedly.
\end{remark}
...
construction, and the Golden Point is the root this program selects
...
% AUTHOR-CHECK (status-report section 3.8): this theorem and Proposition
% gr1:prop:trichotomy fix B, Gamma > 0 as constants, and the proof's
% "Lambda_t >= h_* t pathwise, uses no property of the driver" needs the
% background bounded below along the path.  In the two-channel model
% B_t = sum e^{eps W^(i)_t} has pathwise infimum 0.  The viability
% conclusion P(Lambda_infty < infty) = 0 for rho <= 0 survives (the
% x -> -infty case uses only int B_t dt = infty a.s.); the exponential
% RATE is an annealed statement.  Say which is meant.
...
\end{equation}
whence $\E[T]<\infty$. The same holds for the random background on the
event $\inf_t B_t\ge B_0>0$, with $h_\ast$ evaluated at $B_0$.
...
$\int_0^\infty B_t\,dt=\infty$ a.s.\ by recurrence of the driving
Brownian motions; on any other path at least one channel is bounded below
along a set of infinite measure. In every case $\Lambda_\infty=\infty$.
...
\subsubsection{Two qualifications}
...
with it:
% AUTHOR-CHECK (status-report item 5): this display formerly read
% h^int ~ e^{alpha x} => rho = -alpha, while the provenance footnote in
% gr1:def:anatomy, sch:subsub:internalschedule (q = e^{alpha-beta}),
% ad:sub:toward, and Objection gr:obj:cheap ("beta >= alpha, so that
% rho >= 0") all use rho = -(alpha-beta).  Every downstream use needs
% sign(rho) = sign(beta-alpha), so the display and the two sentences
% after it were changed; the modelling step that changed is "hazard
% scales with unenforced volume PER UNIT OF ENFORCEMENT".  Revert if
% -alpha was intended, but then the four other sites must change.
...
unmistakability condition. The argument therefore both discharges the
...
\subsection{The instrument, and what would refine it}\label{ra:sub:instrument}
...
\section{The Route World: what the strong reading predicts}\label{gc:subsub:routeworld}
...
Everything here is \textbf{[O]}. The section describes what the
hypothesis predicts, states what would distinguish it from its rivals,
and connects the answer to the epistemic ceiling; it argues for the
hypothesis nowhere. What it supplies is the framework's most distinctive
...
system on the interpolation at all. The premise of the reading is
therefore unavailable in principle and not merely unverified.
